...
router.post("/sign-up", async (req: Request, res: Response) => {
  ...
  const existingUser = await db.query(
    "SELECT * FROM marketplace_app.users_info WHERE email = ?",
    [email]
  );

  const existingSitter = await db.query(
    "SELECT * FROM marketplace_app.sitters_info WHERE email = ?",
    [email]
  );
  ...
  if (req.body.type === "customer") {
    ...
    await db.query(
      "INSERT INTO marketplace_app.users_info (first_name, last_name, email, password, type) VALUES (?)",
      [userData]
    );
    ...
    await db.query(
      "INSERT INTO marketplace_app.sitters_info (first_name, last_name, email, password, type, summary, hour_rate, day_rate) VALUES (?)",
      [sitterData]
    );
  }
